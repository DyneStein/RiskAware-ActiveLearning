...
> **See also:** `RESPONSE_TO_REQUESTED_CHANGES.md` for the calibration, statistical-significance, ablation,
...
excellent — 1.0 would be perfect separation, 0.5 is a coin flip). And when that score is allowed to
...
9. **`09_headline_summary.png`** — the two headline percentages (unsafe-accept reduction, FN-rate reduction)
   plotted per pair, with the average as a bar. Good "one glance" summary figure.
...
- **`risk_auroc_by_experiment.csv`** — per-experiment risk AUROC numbers, for checking any single run.
...
- **This is single-seed.** Every number above is from one run per configuration, not averaged over repeated
  ...
  weaker claims go in a paper — see `RESPONSE_TO_REQUESTED_CHANGES.md` for the full significance analysis and what each
...
- **Why unsafe-accepts improved everywhere but FN-rate didn't:** unsafe-auto-accepts is measured on the
  *unlabeled pool*, every round, directly from the escalation decision itself — that's the mechanism the risk
  ...
  from whichever images got labeled — a noisier, more indirect signal. Both matter, but if only one number is
  cited as "the risk score works," `unsafe_auto_accepts` is the more mechanistically honest one.
- **Uncertainty sampling is not a safety mechanism.** The decision-level ablation (see
  `RESPONSE_TO_REQUESTED_CHANGES.md` §9) shows uncertainty-only catches just 12.6% of high-risk pool images — barely
  above the 10.2% obtained by escalating at *random*, despite spending 4,339 labels to do it. The risk route
